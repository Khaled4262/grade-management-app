  ...
  return (
    <div style={css.main}>
      <div style={css.card}>
        ...
        <input style={{ ...css.input, marginBottom:14 }} type="email" placeholder="Email" value={email} disabled={locked} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key==="Enter"&&submit()} />
        <label style={css.label}>Password</label>
        <input style={{ ...css.input, marginBottom:20 }} type="password" placeholder="Password" value={pass} disabled={locked} onChange={(e) => setPass(e.target.value)} onKeyDown={(e) => e.key==="Enter"&&submit()} />
...
function StudentDashboard({ user }) {
  // filter grades for logged-in student
  const myGrades = MOCK_GRADES.filter(
    (g) => g.student === user.name
  );
